...
// Check if user has permission to view parameters
if (!hasPermission('view_parameters')) {
    setFlashMessage("danger", "You don't have permission to view parameters.");
    header("Location: ../../dashboard.php");
    exit();
}

// Get area_id or program_id from URL if available
$areaId = isset($_GET['area_id']) ? intval($_GET['area_id']) : 0;
$programId = isset($_GET['program_id']) ? intval($_GET['program_id']) : 0;

// Get program and area details if IDs are provided
$programDetails = null;
$areaDetails = null;

if ($programId > 0) {
    $programQuery = "SELECT * FROM programs WHERE id = ?";
    ...
    if ($programResult->num_rows > 0) {
        $programDetails = $programResult->fetch_assoc();
    }
}
...
if ($areaId > 0) {
    ...
    if ($areaResult->num_rows > 0) {
        $areaDetails = $areaResult->fetch_assoc();
        $programId = $areaDetails['program_id'];
        $programDetails = [
            'id' => $areaDetails['program_id'],
            'name' => $areaDetails['program_name']
        ];
    }
}

// Add this line before using $whereClauseAdded
$whereClauseAdded = false;

// Find the main query that fetches parameter data and add a left join with a count
$sql = "SELECT p.*, a.name as area_name, a.id as area_id, pr.name as program_name, pr.id as program_id,
        ...
        LEFT JOIN parameter_evidence pe ON p.id = pe.parameter_id";

// Add appropriate filters based on area_id or program_id
if ($areaId > 0) {
    $sql .= " WHERE p.area_level_id = ?";
...
if ($whereClauseAdded) {
    // Add any additional WHERE conditions with AND
    // For example: $sql .= " AND p.status = 'active'";
} else {
    // Add any initial WHERE conditions if no prior WHERE clause exists
    // For example: $sql .= " WHERE p.status = 'active'";
}

// Add GROUP BY to make COUNT work correctly with multiple rows per parameter
$sql .= " GROUP BY p.id";

// Order by p.id instead of p.sort_order
...
<head>
    ...
    <title>Parameters - CCS Accreditation System</title>
        ...
        <div class="main-content">
            ...
            <div class="content">
                ...
                <div class="page-header">
                    <h1>
                        <?php if ($areaDetails): ?>
                            Parameters for <?php echo htmlspecialchars($areaDetails['name']); ?>
                        <?php elseif ($programDetails): ?>
                            Parameters for <?php echo htmlspecialchars($programDetails['name']); ?>
                        <?php else: ?>
                            All Parameters
                        <?php endif; ?>
                    </h1>
                    ...
                    <div class="page-header-actions">
                        <?php if ($areaDetails): ?>
                            <a href="../areas/view.php?id=<?php echo $areaId; ?>" class="btn btn-secondary">
                                <i class="fas fa-arrow-left"></i> Back to Area
                            </a>
                        <?php elseif ($programDetails): ?>
                            <a href="../programs/view.php?id=<?php echo $programId; ?>" class="btn btn-secondary">
                                <i class="fas fa-arrow-left"></i> Back to Program
                            </a>
                        <?php endif; ?>
                        
                        <?php if (hasPermission('add_parameter')): ?>
                            <a href="add.php<?php echo $areaId ? '?area_id=' . $areaId : ''; ?>" class="btn btn-primary">
                ...
                <nav class="breadcrumb-container">
                    <ol class="breadcrumb">
                        ...
                        <?php if ($programDetails): ?>
                            ...
                            <li class="breadcrumb-item"><a href="../programs/view.php?id=<?php echo $programId; ?>"><?php echo htmlspecialchars($programDetails['name']); ?></a></li>
                            
                            <?php if ($areaDetails): ?>
                                <li class="breadcrumb-item"><a href="../areas/view.php?id=<?php echo $areaId; ?>"><?php echo htmlspecialchars($areaDetails['name']); ?></a></li>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <?php if ($paramsResult->num_rows > 0): ?>
                            <div class="table-responsive">
                                <table class="parameters-table">
                                    ...
                                    <tbody>
                                        <?php while ($parameter = $paramsResult->fetch_assoc()): ?>
                                            <tr>
                                                <?php if (!$programDetails): ?>
                                                <td>
                                                    <a href="../programs/view.php?id=<?php echo $parameter['program_id']; ?>">
                                                        <?php echo htmlspecialchars($parameter['program_name']); ?>
                                                    </a>
                                                </td>
                                                <?php endif; ?>
                                                ...
                                                <?php if (!$areaDetails): ?>
                                                <td>
                                                    <a href="../areas/view.php?id=<?php echo $parameter['area_level_id']; ?>">
                                                        <?php echo htmlspecialchars($parameter['area_name']); ?>
                                                    </a>
                                                </td>
                                                <?php endif; ?>
                                                
                                                <td>
                                                    <a href="view.php?id=<?php echo $parameter['id']; ?>">
                                                        <?php echo htmlspecialchars($parameter['name']); ?>
                                                    </a>
                                                </td>
                                                
                                                <td>
                                                    <?php echo strlen($parameter['description']) > 50 ? htmlspecialchars(substr($parameter['description'], 0, 50)) . '...' : htmlspecialchars($parameter['description']); ?>
                                                </td>
                                                
                                                <td><?php echo $parameter['weight']; ?></td>
                                                
                                                <td>
                                                    <?php if ($parameter['status'] == 'active'): ?>
                                                ...
                                                <td class="text-center">
                                                    <span class="badge badge-primary"><?php echo $parameter['evidence_count'] ?? 0; ?></span>
                                                </td>
                                                
                                                <td class="text-center">
                                                    <div class="parameter-actions">
                                                        <a href="view.php?id=<?php echo $parameter['id']; ?>" class="btn-icon" title="View Details">
                                                            <i class="fas fa-eye"></i>
                                                        </a>
                                                        
                                                        <?php if (hasPermission('edit_parameter')): ?>
                                                        <a href="edit.php?id=<?php echo $parameter['id']; ?>" class="btn-icon" title="Edit Parameter">
                                                            <i class="fas fa-edit"></i>
                                                        </a>
                                                        <?php endif; ?>
                                                        
                                                        <?php if (hasPermission('delete_parameter')): ?>
                                                        <a href="delete.php?id=<?php echo $parameter['id']; ?>" class="btn-icon" title="Delete Parameter" onclick="return confirm('Are you sure you want to delete this parameter? This will also delete all associated evidence.');">
                                                            <i class="fas fa-trash-alt"></i>
                                                        </a>
                                                        <?php endif; ?>
                                                        
                                                        <?php if (hasPermission('assign_parameters')): ?>
                                                        <a href="assign_permissions.php?parameter_id=<?php echo $parameter['id']; ?>" class="btn-icon" title="Manage Permissions">
                        ...
                        <?php else: ?>
                            <div class="alert alert-info">
                                <i class="fas fa-info-circle"></i> No parameters found.
                                <?php if (hasPermission('add_parameter')): ?>
                                    <a href="add.php<?php echo $areaId ? '?area_id=' . $areaId : ''; ?>" class="alert-link">Add your first parameter</a>.
                                <?php endif; ?>
                            </div>
                        <?php endif; ?>
                    </div>
