...
// Check if user has permission to edit sub-parameters
if (!hasPermission('edit_parameter')) {
    setFlashMessage("danger", "You don't have permission to edit sub-parameters.");
...
$query = "SELECT * FROM sub_parameters WHERE id = ?";
...
$parameterQuery = "SELECT name FROM parameters WHERE id = ?";
$parameterStmt = $conn->prepare($parameterQuery);
$parameterStmt->bind_param("i", $parameterId);
$parameterStmt->execute();
$parameterResult = $parameterStmt->get_result();
$parameterName = ($parameterResult->num_rows > 0) ? $parameterResult->fetch_assoc()['name'] : 'Unknown Parameter';

// Get available columns in sub_parameters table
$tableColumnsQuery = "SHOW COLUMNS FROM sub_parameters";
...
if (!isset($subParameter['weight'])) {
    $subParameter['weight'] = 0;
}
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $errors = [];
    if (empty($name)) {
        $errors[] = "Sub-parameter name is required.";
    }
    ...
    if (empty($errors)) {
        ...
        $updateTypes .= "i";
        $updateParams[] = $subParameterId;
        
        // Create the update query
        $updateQuery = "UPDATE sub_parameters SET " . implode(", ", $updateFields) . " WHERE id = ?";
        ...
        if ($updateStmt->execute()) {
            ...
            $activityType = "sub_parameter_updated";
            $activityDescription = "Updated sub-parameter: $name";
            ...
            setFlashMessage("success", "Sub-parameter updated successfully.");
            header("Location: view_sub_parameter.php?id=" . $subParameterId);
            exit();
        } else {
            $errors[] = "Failed to update sub-parameter. Error: " . $conn->error;
        }
    }
}

// Get evidence count for this sub-parameter
$evidenceCount = 0;
...
if ($evidenceTableExists) {
    try {
        // Check if the 'sub_parameter_id' column exists in the evidence table
        ...
        if ($evidenceColumnsResult->num_rows > 0) {
            $evidenceQuery = "SELECT COUNT(*) as count FROM evidence WHERE sub_parameter_id = ?";
...
<head>
    ...
    <title>Edit Sub-Parameter - CCS Accreditation System</title>
        ...
        <div class="main-content">
            ...
            <div class="content">
                ...
                <div class="page-header">
                    <div>
                        <h1>Edit Sub-Parameter</h1>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="../../dashboard.php">Dashboard</a></li>
                            <li class="breadcrumb-item"><a href="list.php">Parameters</a></li>
                            <li class="breadcrumb-item"><a href="view.php?id=<?php echo $parameterId; ?>"><?php echo htmlspecialchars($parameterName); ?></a></li>
                            <li class="breadcrumb-item active">Edit Sub-Parameter</li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="row">
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title"><i class="fas fa-edit"></i> Edit Sub-Parameter</h5>
                            </div>
                            <div class="card-body">
                                <form method="POST" action="">
                                    <div class="form-group">
                                        <label for="name">Sub-Parameter Name <span class="text-danger">*</span></label>
                                        <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($subParameter['name']); ?>" required>
                                    </div>
                                    
                                    <?php if (isset($availableColumns['weight'])): ?>
                                    <div class="form-group">
                                        <label for="weight">Weight <span class="text-danger">*</span></label>
                                        <input type="range" class="form-range" id="weight" name="weight" min="0" max="100" step="0.5" value="<?php echo $subParameter['weight']; ?>">
                                        <div class="weight-indicator">
                                            <div class="weight-bar">
                                                <div class="weight-progress" style="width: <?php echo $subParameter['weight']; ?>%;"></div>
                                            </div>
                                            <span class="range-value weight-value"><?php echo $subParameter['weight']; ?></span>%
                                        </div>
                                        <small class="form-text text-muted">Weight represents the importance of this sub-parameter relative to others. Total weights for all sub-parameters should equal 100%.</small>
                                    </div>
                                    <?php endif; ?>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" id="description" name="description" rows="4"><?php echo htmlspecialchars(isset($subParameter['description']) ? $subParameter['description'] : ''); ?></textarea>
                                    </div>
                                    <?php endif; ?>
                                    
                                    <div class="mt-4 d-flex justify-content-between">
                                        <a href="view_sub_parameter.php?id=<?php echo $subParameterId; ?>" class="btn btn-secondary">
                                            <i class="fas fa-arrow-left"></i> Cancel
                                        </a>
                                        <button type="submit" class="btn btn-primary">
                                            <i class="fas fa-save"></i> Update Sub-Parameter
                                        </button>
                                    </div>
                                </form>
                            </div>
                    ...
                    <div class="col-md-4">
                        <div class="card">
                            <div class="card-header">
                                <h5 class="card-title"><i class="fas fa-info-circle"></i> Sub-Parameter Info</h5>
                            </div>
                            <div class="card-body">
                                ...
                                <div class="stats-card" style="background-color: rgba(74, 144, 226, 0.1);">
                                    ...
                                    <div class="stats-details">
                                        <div class="stats-value"><?php echo isset($subParameter['created_at']) ? date('M d, Y', strtotime($subParameter['created_at'])) : 'N/A'; ?></div>
                                ...
                                <div class="mt-3">
                                    <h6 class="font-weight-bold mb-3">Parent Parameter:</h6>
                                    <div class="card bg-light mb-3">
                                        <div class="card-body p-3">
                                            <h6 class="mb-1"><?php echo htmlspecialchars($parameterName); ?></h6>
                                            <a href="view.php?id=<?php echo $parameterId; ?>" class="btn btn-sm btn-outline-primary mt-2">
                                                <i class="fas fa-eye"></i> View Parameter
                                            </a>
                                        </div>
                                    </div>
                                    
                                    <h6 class="font-weight-bold mt-4 mb-3">Related Actions:</h6>
                                    <div class="d-flex flex-column">
                                        <a href="view_sub_parameter.php?id=<?php echo $subParameterId; ?>" class="btn btn-sm btn-outline-primary mb-2">
                                            <i class="fas fa-eye"></i> View Sub-Parameter
                                        </a>
                                        
                                        <?php if (hasPermission('add_evidence')): ?>
                                        <a href="../evidence/add.php?sub_parameter_id=<?php echo $subParameterId; ?>" class="btn btn-sm btn-outline-success mb-2">
                                        ...
                                        <button type="button" class="btn btn-sm btn-outline-danger" data-toggle="modal" data-target="#deleteModal">
                                            <i class="fas fa-trash-alt"></i> Delete Sub-Parameter
                                        </button>
                                        <?php endif; ?>
                                    </div>
                                </div>
        ...
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Are you sure you want to delete this sub-parameter? This action cannot be undone.</p>
                    
                    <?php if ($evidenceCount > 0): ?>
                    <div class="alert alert-warning">
                        <i class="fas fa-exclamation-triangle mr-2"></i>
                        <strong>Warning:</strong> This sub-parameter has <?php echo $evidenceCount; ?> evidence file(s) that will be deleted.
                    </div>
                    <?php endif; ?>
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
                    <a href="delete_sub_parameter.php?id=<?php echo $subParameterId; ?>" class="btn btn-danger">Delete Sub-Parameter</a>
                </div>
            </div>
        </div>
    </div>
