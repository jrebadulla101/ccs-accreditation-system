...
if (!hasPermission('edit_area')) {
    setFlashMessage("danger", "You don't have permission to edit areas.");
    header("Location: list.php");
    exit();
}

// Check if area ID is provided
if (!isset($_GET['id']) || empty($_GET['id'])) {
    setFlashMessage("danger", "No area ID provided.");
    header("Location: list.php");
    exit();
}
...
// Get area details
$query = "SELECT * FROM area_levels WHERE id = ?";
...
$area['weight'] = isset($area['weight']) ? $area['weight'] : 0;

// Check the table structure to determine available columns
$tableColumnsQuery = "SHOW COLUMNS FROM area_levels";
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (empty($errors)) {
        ...
        // Include program_id if available
        if (isset($availableColumns['program_id'])) {
        ...
        $updateQuery = "UPDATE area_levels SET " . implode(", ", $updateFields) . " WHERE id = ?";
        ...
        if ($updateStmt->execute()) {
            ...
            $activityType = "area_updated";
            $activityDescription = "Updated area: $name";
            ...
            setFlashMessage("success", "Area updated successfully.");
            header("Location: list.php");
            exit();
        } else {
            $errors[] = "Failed to update area. Error: " . $conn->error;
        }
    }
}

// Get all programs for the dropdown
$programsQuery = "SELECT id, name FROM programs ORDER BY name ASC";
...
// Check parameters table structure for the area reference column
$paramStructureQuery = "SHOW TABLES LIKE 'parameters'";
$parametersExist = $conn->query($paramStructureQuery)->num_rows > 0;
...
if ($parametersExist) {
    ...
    if ($paramColumns) {
        while ($column = $paramColumns->fetch_assoc()) {
            // Look for area_id or area_level_id or similar
...
// Check user_permissions table existence and structure
$checkUserPermissions = "SHOW TABLES LIKE 'user_permissions'";
$userPermissionsExists = $conn->query($checkUserPermissions)->num_rows > 0;
$permissionAreaColumn = 'area_id'; // Default

if ($userPermissionsExists) {
    $permStructureQuery = "SHOW COLUMNS FROM user_permissions";
    $permColumns = $conn->query($permStructureQuery);
    
    if ($permColumns) {
        while ($column = $permColumns->fetch_assoc()) {
            if ($column['Field'] == 'area_level_id') {
                $permissionAreaColumn = 'area_level_id';
                break;
            }
        }
    }
...
<head>
    ...
    <title>Edit Area - CCS Accreditation System</title>
            ...
            <div class="sidebar-menu">
                <ul>
                    ...
                    <li class="menu-item active">
                        ...
                        <ul class="submenu">
                            <li><a href="list.php" class="submenu-link">View All Areas</a></li>
                            <?php if (hasPermission('add_area')): ?>
                            <li><a href="add.php" class="submenu-link">Add New Area</a></li>
                            <?php endif; ?>
                        </ul>
                    </li>
        ...
        <div class="main-content">
            ...
            <div class="content">
                ...
                <div class="page-header">
                    <div>
                        <h1>Edit Area Level</h1>
                        <ul class="breadcrumb">
                            <li class="breadcrumb-item"><a href="../../dashboard.php">Dashboard</a></li>
                            <li class="breadcrumb-item"><a href="list.php">Area Levels</a></li>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title"><i class="fas fa-edit"></i> Edit Area Level</h5>
                    </div>
                    <div class="card-body">
                            ...
                            <div class="row">
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control" id="name" name="name" value="<?php echo htmlspecialchars($area['name']); ?>" required>
                                    </div>
                                    
                                    <?php if (isset($availableColumns['program_id'])): ?>
                                    <div class="form-group">
                                        <label for="program_id">Program</label>
                                        <select class="form-control" id="program_id" name="program_id">
                                            <option value="0">-- Select Program --</option>
                                            <?php foreach ($programs as $program): ?>
                                                <option value="<?php echo $program['id']; ?>" <?php echo (isset($area['program_id']) && $area['program_id'] == $program['id']) ? 'selected' : ''; ?>>
                                ...
                                <div class="col-md-6">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <input type="range" class="form-range" id="weight" name="weight" min="0" max="100" step="0.5" value="<?php echo $area['weight']; ?>">
                                        <div class="weight-indicator">
                                            <div class="weight-bar">
                                                <div class="weight-progress" style="width: <?php echo $area['weight']; ?>%;"></div>
                                            </div>
                                            <span class="range-value weight-value"><?php echo $area['weight']; ?></span>%
                                        </div>
                                        <small class="form-text text-muted">Weight represents the importance of this area relative to others. Total weights for all areas should equal 100%.</small>
                                    </div>
                                    <?php endif; ?>
                                </div>
                                ...
                                <div class="col-md-12">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <textarea class="form-control" id="description" name="description" rows="4"><?php echo htmlspecialchars(isset($area['description']) ? $area['description'] : ''); ?></textarea>
                                    </div>
                                    <?php endif; ?>
                                </div>
                            </div>
                ...
                <!-- Area Parameters Information Card -->
                <div class="card">
                    <div class="card-header">
                        <h5 class="card-title"><i class="fas fa-info-circle"></i> About This Area</h5>
                    </div>
                    <div class="card-body">
                        <?php
                        // Safely get parameters count
                        $paramsCount = 0;
                        if ($parametersExist) {
                            try {
                                $paramsQuery = "SELECT COUNT(*) as count FROM parameters WHERE $areaReferenceColumn = ?";
                                $paramsStmt = $conn->prepare($paramsQuery);
                                $paramsStmt->bind_param("i", $areaId);
                                $paramsStmt->execute();
                                $paramsResult = $paramsStmt->get_result();
                                $paramsCount = $paramsResult->fetch_assoc()['count'];
                            } catch (Exception $e) {
                        ...
                        if ($evidenceTableExists && $parametersExist) {
                            try {
                                // Get evidence count for this area
                                $evidenceQuery = "SELECT COUNT(*) as count FROM evidence 
                                                WHERE parameter_id IN (SELECT id FROM parameters WHERE $areaReferenceColumn = ?)";
                        ...
                        // Get users with access to this area
                        $usersCount = 0;
                        if ($userPermissionsExists) {
                            try {
                                $usersQuery = "SELECT COUNT(DISTINCT user_id) as count FROM user_permissions WHERE $permissionAreaColumn = ?";
                                $usersStmt = $conn->prepare($usersQuery);
                                $usersStmt->bind_param("i", $areaId);
                                $usersStmt->execute();
                        ...
                        <div class="row">
                            <div class="col-md-4">
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <div>
                                        <h5 class="mb-0"><?php echo $paramsCount; ?> Parameters</h5>
                                        <small class="text-muted">Associated with this area</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <div>
                                        <h5 class="mb-0"><?php echo $evidenceCount; ?> Evidence Files</h5>
                                        <small class="text-muted">Uploaded for this area</small>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="col-md-4">
                                <div class="d-flex align-items-center mb-3">
                                    ...
                                    <div>
                                        <h5 class="mb-0"><?php echo $usersCount; ?> Users</h5>
                                        <small class="text-muted">With access to this area</small>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="mt-3">
                            ...
                            <div class="d-flex flex-wrap mt-2">
                                <a href="../parameters/list.php?area_id=<?php echo $areaId; ?>" class="btn btn-sm btn-outline-primary mr-2 mb-2">
                                    <i class="fas fa-list"></i> View Parameters
                                </a>
                                <?php if (hasPermission('add_parameter')): ?>
                                <a href="../parameters/add.php?area_id=<?php echo $areaId; ?>" class="btn btn-sm btn-outline-success mr-2 mb-2">
                                    <i class="fas fa-plus"></i> Add Parameter
                                </a>
                                <?php endif; ?>
                                <?php if (hasPermission('manage_permissions')): ?>
                                <a href="../permissions/manage.php?area_id=<?php echo $areaId; ?>" class="btn btn-sm btn-outline-secondary mr-2 mb-2">
                                    <i class="fas fa-key"></i> Manage Access
                                </a>
                                <?php endif; ?>
                                <?php if (hasPermission('view_reports')): ?>
                                <a href="../reports/area.php?id=<?php echo $areaId; ?>" class="btn btn-sm btn-outline-info mr-2 mb-2">
